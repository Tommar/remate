    ...
    $B(document).ready(function(){
        ...
        $B(moduleID + ' .jcarousel').jcarousel({
            initCallback: function(carousel, state){
                 ...
                 ************************************************************************/
                var hammer = new Hammer(carousel.list.parent()[0],{prevent_default: true, not_prevent_tags:{id:[], className:['bt-title'],tagName:[]}});
                var carouselWidth = carousel.list.parent().width();
                var windowPosition = $B('html,body').offset().top;
    			$B(window).scroll(function(){
    				windowPosition = $B('html,body').offset().top;
    			});
                ...
                hammer.ontap = function (ev){
                    ...
                    if(target.tagName.toLowerCase() == 'a' && target.parentNode.className != 'bt-inner' ){
                ...
                <?php if($effect != 'fade'){ ?>
	                ...
	                hammer.ondrag = function (ev){
		                if(carousel.options.onAnimate || ev.originalEvent.target.className=='bt-title'){ return;}
	                    if(!carousel.options.vertical){
	                        ...
	                        if(ev.direction == 'up'){
	                        	carousel.list.css('top', carousel.options.posAfterAnimate - ev.distance);
	                        }
	                    ...
	                    if(ev.distance > 100){
	                        if(!carousel.options.vertical){
	                            ...
	                            if(ev.direction == 'up'){
	                            	if(carousel.first == carousel.options.size){
	                            		carousel.options.onAnimate = true;
										carousel.list.animate({top: carousel.options.posAfterAnimate}, 250, function (){ carousel.options.onAnimate = false;});
									}else{
										btContentShowcaseAjustHeight(carousel, originalHeight);
										carousel.pauseAuto();
										carousel.options.onAnimate = true;
	                            ...
	                            if(ev.direction == 'down'){
	                            	if(carousel.first == 1){
	                            		carousel.options.onAnimate = true;
										carousel.list.animate({top: 0}, 250,function(){carousel.options.onAnimate = false;});
									}else{
										btContentShowcaseAjustHeight(carousel, originalHeight);
										carousel.pauseAuto();
										carousel.options.onAnimate = true;
              ...
              <?php  } ?>
                ...
                hammer.onswipe = function (ev){
                    if(carousel.options.onAnimte || ev.originalEvent.target.className=='bt-title'){return;}
                    if(!carousel.options.vertical){
	                	if(ev.direction == 'left'){
							if(carousel.first == carousel.options.size){
								carousel.options.onAnimate = true;
						...
						if(ev.direction == 'right'){
							if(carousel.first == 1){
								...
								<?php }else{ ?>
									carousel.options.onAnimate = true;
									carousel.animate(carousel.pos(carousel.first - 1) );
								<?php }?>
								carousel.stopAuto();
				                carousel.options.auto = 10000;
		                    	}
						...
						if(ev.direction == 'up'){
							$B('html,body').animate({scrollTop: -windowPosition + 500},500);
						}
						if(ev.direction == 'down'){
							$B('html,body').animate({scrollTop: -windowPosition - 500},500);
						}
                    	...
                    	if(ev.direction == 'up'){
							if(carousel.first == carousel.options.size){
								...
								carousel.pauseAuto();
								carousel.options.onAnimate = true;
								carousel.animate(carousel.pos(carousel.first + 1) + ev.distance);
								carousel.stopAuto();
				                carousel.options.auto = 10000;
		                    	}
							}
