...
require_once JPATH_ROOT . '/modules/mod_btimagegallery/helpers/images.php';
...
class JFormFieldAjax extends JFormField {
    ...
    protected function getInput() {
        ...
        $this->saveDir = JPATH_ROOT . '/modules/mod_btimagegallery/images';
                        ...
                        if (JRequest::getString('action') == 'get') {
                            ...
                            if (count($photos) > 0) {
                                ...
                                if (!isset($this->result['message']) || $this->result['message'] == '')
                                    $this->result["message"] = JText::_('MOD_BTIMAGEGALLERY_ERROR_NO_IMAGE');
                            }
                            return json_encode($this->result);
	...
	function getFlickrPhotosets(){
		...
		if (count($arrOptions) > 0) {
			...
			$objOption = new stdClass();
			$objOption->value = 0;
			$objOption->text = JText::_('MOD_BTIMAGEGALLERY_FLICKR_ALL_PHOTOSETS');
			$arrOptions[] = $objOption;
			$arrOptions = array_reverse($arrOptions);
		}
		return $arrOptions;
	...
	function getPicasaAlbums(){
		...
		if (isset($sxml) && $sxml) {
			...
			if (count($arrOptions) > 0) {
				...
				$objOption = new stdClass();
				$objOption->value = 0;
				$objOption->text = JText::_('MOD_BTIMAGEGALLERY_PICASA_ALL_ALBUMS');
				$arrOptions[] = $objOption;
				$arrOptions = array_reverse($arrOptions);
			}
		} else {
	...
	function getArticle(){
		...
		if (!$user->id) {
			$this->result['message'] = JText::_('MOD_BTIMAGEGALLERY_ERROR_NOT_LOGIN');
		...
		if (in_array($extension, $this->allowedExtensions)) {
			...
			if (!in_array($filename, $this->items)) {
				...
				if (!JFile::upload($file, "{$this->saveDir}/tmp/original/{$filename}")) {
					$this->result['message'] = JText::_('MOD_BTIMAGEGALLERY_ERROR_COULD_NOT_SAVE');
					$validated = false;
				} else {
				...
				$this->result['message'] = JText::_('MOD_BTIMAGEGALLERY_FILE_EXISTED');
				$validated = false;
			}
		} else {
			$this->result['message'] = JText::_('MOD_BTIMAGEGALLERY_FILE_EXTENSION_INVALID');
			$validated = false;
		}
		if ($validated) {
			$this->result["success"] = true;
		...
		if (is_dir($jFolderPath)) {
			...
			while ($filename !== false) {
				...
				if (filetype($jFolderPath .'/'. $filename) == "file") {
					...
					//var_dump(in_array(strtolower($fileInfo["extension"]), $this->allowedExtensions));
					//var_dump(!JFile::exists($this->saveDir . "/tmp/manager/{$hashedName}.{$fileInfo["extension"]}"));
					//var_dump(!in_array($hashedName . '.' . $fileInfo["extension"], $this->items));
					if (
							$file
			...
			$this->result['message'] = JText::_('MOD_BTIMAGEGALLERY_ERROR_DIRECTORY');
			$validated = false;
		}
	}
	...
	function getImagesFromPhoca(& $photos){
		...
		if (!$helper->checkPhocaComponent()) {
			$this->result["message"] = JText::_('MOD_BTIMAGEGALLERY_COM_PHOCA_NOT_EXIST');
		} else {
	...
	function getImagesFromJoomGallery(& $photos){
		...
		if (!$helper->checkJGalleryComponent()) {
			$this->result["success"] = false;
			$this->result["message"] = JText::_('MOD_BTIMAGEGALLERY_COM_JOOMGALLERY_NOT_EXIST');
		} else {
	...
	function delete(){
		...
		try {
			...
			JFile::delete($this->saveDir . '/crop/' . $file);
	...
	function isWritable(){
		if (
			...
			|| !is_writable($this->saveDir . '/crop')
			|| !is_writable($this->saveDir . '/thumbnail')
			|| !is_writable($this->saveDir . '/manager')
			|| !is_writable($this->saveDir . '/original')
		) {
			$this->result['message'] = JText::_('MOD_BTIMAGEGALLERY_ERROR_SAVE_DIR_NOT_WRITABLE');
			return false;
		}
		return true;
	}
