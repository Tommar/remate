...
<div class="defaultLayout" style="width:<?php echo $moduleWidthWrapper;?>; ">
	...
	<div id="btcontentshowcase<?php echo $module->id; ?>"  class="bt-contentshowcase<?php echo $moduleclass_sfx ? ' bt-contentshowcase'.$params->get('moduleclass_sfx'):'';?>">
		...
		<?php 
			...
			$add_style= "border: 0; float:left; width:75%;";
		?>
		<div class="swap-h3">
		<h3>
                    <?php if($params->get('content_title_link')) {?>
                        <a href="<?php echo $params->get('content_title_link');?>"><?php 
							$str = $params->get('content_title');							
							for($i=0;$i <= strlen($str);$i++){
								if(substr($str,$i,1)== ' '){							
									echo substr($str,0,-$i-1).'<span>'.substr($str,-$i-1).'</span>';
									break;
								}
							}
						?></a>
		...
		<?php } ?>
		<div class="swap-item"  style="<?php echo $add_style;?>">
					...
					<?php if($show_category_name_as_link) : ?>
						...
						<a target="<?php echo $openTarget; ?>"
							...
							title="<?php echo $row->title;?>" href="<?php echo $modal?$row->mainImage:$row->link;?>">
							<img <?php echo $imgClass ?> src="<?php echo $row->thumbnail; ?>" alt="<?php echo $row->title?>"  style="width:<?php echo $thumbWidth ;?>px;" title="<?php echo $row->title?>" />							
							<?php  if($moduleclass_sfx == '_last'){ ?>
								<div class="arrow"><div class="arrow1"></div></div>		
							<?php } ?>
						</a>
						</div>
                                                <?php } ?>
						...
						<?php if(isset($row->extra_fields) && count($row->extra_fields) != 1): ?>
						<?php foreach ($row->extra_fields as $field){				
							
							if(count($field) ==0) continue;	?>
							<div class="extrafield-row <?php echo preg_replace("/[^a-zA-Z0-9]/", "", strtolower($field->name)) ?>" >
							<?php if(strtolower($field->type) != 'measurement') : ?>
							...
							switch($field->type){
								...
								case'image':
									...
									break;
								case'measurement':							
									?><span class="extrafield-value swap-bt-price"><span class="bt-price"><?php echo $field->value.' '.$field->default_value[1]; ?></span></span><?php 
									break;
								case'dropdown':
		...
		<?php if($moduleclass_sfx == "_feature"): ?>
