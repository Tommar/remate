        ...
        <div class="btp-slideshow">
			<div class="box_skitter box_skitter_large">
				<ul>
					...
					?>
					<li><img alt="<?php echo htmlspecialchars($image->title) ?>" class="block" 	src="<?php echo $src_image_large; ?>" rel="<?php echo $src_image_thumb;?>" />
                        ...
                        <?php if ($this->params->get('show_url') && $this->item->url) :	?>
							  <div class="bg-btp-detail-link">
					            <div class="btp-detail-link">
					                <a target="_blank" title="<?php echo JText::_('COM_BT_PORTFOLIO_VISIT_SITE'); ?>" class="visit-site" href="<?php  echo $this->item->url; ?>"><?php echo JText::_('COM_BT_PORTFOLIO_VISIT_SITE'); ?>
					                </a>
				                </div>
				              </div>
                       <?php endif;	?>
	...
	<?php endif; ?>
						...
						<div class="btp-comment-item-head">
	                        ...
	                        <div class="in-content-comment">
								...
								<?php if ($this->params->get('show_title')) : ?>
									<span class="comment-title"> <?php echo htmlspecialchars($item->title); ?></span>
								<?php endif;?>
		...
		<div class="btp-comment-fom">
        	<h3 class="review-form-title">
				<?php echo JText::_('COM_BT_PORTFOLIO_WRITE_A_REVIEW'); ?>
			</h3>
			<?php if ($this->params->get('allow_guest_comment') || $this->user->id) { ?>
			<form id="btp-form-comment"	action="<?php echo JRoute::_('index.php?option=com_bt_portfolio&task=comment.comment'); ?>" method="post" class="form-validate">
				<?php foreach ($this->comment['form']->getFieldsets() as $fieldset) : ?>
					...
					<?php endforeach; ?>
				<?php endforeach; ?>
					<div class="btl-submit-comment-btp">
					...
					<input type="hidden" name="return" value="<?php echo base64_encode($this->uri->toString(array('path', 'query', 'fragment'))) ?>">
						<span><button type="submit" class="button validate">
			...
			<div class="login-first">
				<a href="<?php echo JRoute::_('index.php?option=com_users&view=login&return='.base64_encode($this->uri->toString(array('path', 'query', 'fragment')))); ?>">
					<?php echo JText::_('COM_BT_PORTFOLIO_LOGIN_FIRST'); ?>
				</a>
			</div>
			<?php
				}
