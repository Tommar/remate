...
?>
		...
		<?php
			...
			<h3 class="btp-comment-head">
				<?php echo $this->item->review_count.' '; echo $this->item->review_count > 1 ? JText::_('COM_BT_PORTFOLIO_REVIEWS') : JText::_('COM_BT_PORTFOLIO_REVIEW') ?>
			</h3>

			<!-- Comment lits -->
			<div id="btp-comment-list">
            <?php foreach ($this->comment['data'] as $item){
				$classRow = ($i%2)?'even' : 'odd';
            	$i++;
             ?>
				<div class="btp-comment-item">
					<div class="<?php echo $classRow;?> <?php if($i==0) echo 'comment-first'; else if ($i == $total) echo  'comment-last';?> <?php echo $item->admin? 'comment-admin':''; ?>">
